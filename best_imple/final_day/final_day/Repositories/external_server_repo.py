...
class ExternalServerRepository:
    def get_all(self):
        ...
        cursor.execute("SELECT * FROM external_servers ORDER BY server_id")
    ...
    def get_by_id(self, server_id: int):
        conn = DbConnection.get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM external_servers WHERE server_id = %s", (server_id,))
    ...
    def update(self, server_id: int, server: ExternalServerUpdate):
        conn = DbConnection.get_db_connection()
        cursor = conn.cursor()
